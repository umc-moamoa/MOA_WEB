...
const fetchSurvey = () => {
    ...
    fetch(
        `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2C%20statistics&chart=mostPopular&maxResults=4&regionCode=kr&key=${API_KEY}`,
        requestOptions
    )
...
function SurveyListTemplate (data) {
    const SurveyItem = `<div id="main1">
                            <div class="one-container1">
                                <a id="title1" href="../html/moa1.html" data-id=${data.id}>  ${data.snippet.title}  </a>
                            </div>
                            ...
                            <div class="three-container1">
                                <span id="point1">  ${data.snippet.channelTitle}  </span>
                            </div>
                        </div>
    `;
